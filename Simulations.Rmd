...
```{r library imports, echo=1:6, message=FALSE, warning=FALSE}
...
First, we generate data, either with the expected effects (score above chance, no effect of age), or with a true null effect (no preference). For the size of the dataset, we take the minimum number of labs we expect to participate, with the number of participants we ask from each lab.

For the "true effect" data set, we use the estimated true effect size from a recent meta-analysis (Margoni & Surian, 2018) for the mean probability of chosing the helper versus hinderer character, and the 95% Confidence Interval and average sample size in studies included in Margoni and Surian (2018) to compute the standard deviation (by using the formula for CIs).
...
```{r initialise_models full_raw_age, eval=FALSE}
...
if(run_models){
  ...
  n_previous <- ifelse(file.exists(filename.bf), length(read_lines(filename.bf))-1, 0)
...
estimates.raw_age.full <- read_csv(paste0(save_path, "estimates_full.csv")) %>%
  mutate_at(vars(Parameter), as_factor) %>% 
  drop_na()
estimates.ordered <- estimates.raw_age.full %>%
  subset(Parameter == "Intercept", select = c(Estimate, sim_id)) %>%
  arrange(Estimate) %>%
  rownames_to_column(var = "ordered") %>%
  mutate_at("ordered", as.numeric) %>%
  select(c(sim_id, ordered))
estimates.raw_age.full <- left_join(estimates.raw_age.full, estimates.ordered)
# True simulated effects
true_effects <- tibble(Parameter = c("Intercept", "age_days"),
                       Value = c(.5753641, 0))
# Plot
ggplot(estimates.raw_age.full,
       aes(x = ordered, y = Estimate,
...
## Using scaled and centred `z_ager_days`
...
## Read Bayes factors
bf.scaled_age <- read_csv(paste0(save_path, "bayes_factors.csv"))
```
...
ggplot(estimates.scaled_age.full,
  ...
  facet_grid(cols = vars(Parameter), scales = "free_x") +
  geom_hline(data = true_effects, aes(yintercept = Value), linetype = "dashed") +
  geom_pointrange(fatten = .5, size = .2)
```
...
bf.all <- bind_rows(bf.raw_age.tmp, bf.scaled_age.tmp) %>%
  mutate(log_b.value = log1p(b.value))
# Raincloud plot
ggplot(bf.all, aes(x = age, y = log_b.value,
                   colour = age, fill = age)) +
...
evidence <- bf.all %>%
  group_by(age) %>%
  summarise(p_H1 = mean(b.value >= 3),
            p_H0 = mean(b.value <= 1/3))
p_H1.raw_age <- evidence %>% subset(age == "raw age", p_H1) %>% as.numeric()
p_H0.raw_age <- evidence %>% subset(age == "raw age", p_H0) %>% as.numeric()
```

Clearly, we see that models using raw `age_days` do not detect the true simulated effect, with only `r p_H1.raw_age*100`% of the models resulting in a $b\geq3$, and sometimes even give evidence for the null with a Bayes factor $b\leq {}^1\!/_3$ (`r p_H0.raw_age*100`%). On the other hand, models using scaled and centred `z_age_days` reliably provided robust evidence for the true simulated effect.

Thus, we use `z_age_days` in the MB4 analysis script.

# Evidence for a true null effect for age

Another important question in the MB4 study is whether or not there is an effect of age on probability of choosing the helper vs. hinderer character. Here, we check how reliably the Bayesian analysis proposed (using scaled and centred `z_age_days`) provides evidence for a true null effect of age.
...
```{r no_age models, echo=FALSE, message=FALSE, warning=FALSE}
...
## Read Bayes factors
bf.null_age <- read_csv(paste0(save_path, "bayes_factors.csv"))
```
...
# Get Bayes factors into a proper tibble
bf.null_age <- bf.null_age %>%
  mutate(test = "no age",
         log_b.value = log1p(b.value))
# Raincloud plot
ggplot(bf.null_age, aes(x = test, y = log_b.value,
...
```{r echo=FALSE, message=FALSE, warning=FALSE}
evidence <- bf.null_age %>%
  summarise(p_H1 = mean(b.value >= 3),
            p_H0 = mean(b.value <= 1/3))
p_H1.null_age <- evidence %>% select(p_H1) %>% as.numeric()
p_H0.null_age <- evidence %>% select(p_H0) %>% as.numeric()
```
...
bf.true_null <- bf.true_null %>%
  mutate(test = "true null",
         log_b.value = log1p(b.value))
# Raincloud plot
ggplot(bf.true_null, aes(x = test, y = log_b.value,
                   colour = test, fill = test)) +
  theme_bw() + ylab("Log Bayes factor [log(b+1)]") + coord_flip() +
  theme(legend.position = "top",
        axis.title.y = element_blank(),
...
```{r eval=FALSE}
# Model for H1
chose_helper ~ 1 + condition + z_age_days + (1 + condition + z_age_days | lab_id)
# Model for H0
chose_helper ~ 1 +             z_age_days + (1 +             z_age_days | lab_id)
```
...
pwr.design <- tibble(design = as_factor(c("_ctrl_", "_no-ctrl_"))) %>%
  ...
  mutate(beta = map2_dbl(design, n10_infants, function(design, n){
    read_csv(paste0(save_path, "bf", design, n, ".csv")) %>%
      drop_na() %>%
      deframe() %>%
      {if_else(length(.)<20, NA_real_, sum(.>3)/length(.))} # Remove if too few samples
    }))
...
We see that we would need around 135 participants to reach a power of 80% without a non-social control group, but more than 500 participants in total to reach the same power with a non-social control group.

## Non-bayesian simulations

The important increase in number of participants to test for one additional condition we saw above seems surprising, however a similar ratio is observed with a simpler frequentist simulation:

```{r message=FALSE, warning=FALSE}
run_models <- F
if(run_models){
  n_sim <- 1000
  pwr.ctrl <- expand_grid(n_total = seq(50,500,10), 
                          i = 1:n_sim) %>%
    mutate(idx = 1:n()) %>%
    split(.$idx) %>%
    future_map_dfr(function (df) {
      cntl_sim <- tibble(choice = c(rbinom(df$n_total/2, 1, pr_helper_mean), 
                                    rbinom(df$n_total/2, 1, .5)), 
                         condition = c(rep("social", df$n_total/2), 
                                       rep("nonsocial", df$n_total/2)))
      nocntl_sim <- tibble(choice = rbinom(df$n_total, 1, pr_helper_mean))
  ...
  write_csv(pwr.ctrl, paste0(save_path, "glm_sims.csv"))
}else{
  pwr.ctrl <- read_csv(paste0(save_path, "glm_sims.csv"))
}
...
pwr.ctrl %>%
  ...
  mutate(condition = ifelse(condition == "cntl_power",
                            "With Social Control",
                            "No Social Control")) %>%
...
Importantly, these simulations are much faster to run, and given the similar results in terms of power, we will use those to answer our next question, and extrapolate the results to Bayesian statistics.

# Impact of between-lab variability on statistical power

One important question that we did not address so far in these simulations is of the possible impact of between-lab variation on our results. To answer this question, we take the estimated effect sizes and confidence intervals width reported in Margoni and Surian (2018), treated in the same way as before to obtain a standard deviation, to add this variability to our data. To compute the standard deviation from the CI, we assume 16 participants for each lab, which is a conservative measures as more infants would result in a smaller standard deviation value computed from the CI.

```{r}
lab_var <- tribble(
  ~lab_id, ~estimate, ~lower, ~upper,
  "lab1",  .92, .80, .99,
  "lab2",  .75, .54, .96,
  "lab3",  .69, .51, .88,
  "lab4",  .77, .60, .94,
  ...
  mutate(CI_spread = (upper-lower)/2,
         stdev = CI_spread * sqrt(16) / 1.96) %>% # We assume 16 infants for each lab
  select(lab_id, estimate, stdev) %>%
  mutate_at(vars(lab_id), as_factor)
```

For the simulations, we randomly sample a subset of these labs according to the number of participants, so that we have at least 16 participants for each lab, with a maximum number of 20 participating labs.

```{r message=FALSE, warning=FALSE}
save_path <- "simulation_results/lab_var/"
run_models <- F
if(run_models){
  n_sims <- 1000
  pwr.lab_var <- expand_grid(n_total = seq(50,500,10),
                             i = 1:n_sims) %>%
    mutate(idx = 1:n()) %>%
    split(.$idx) %>%
    future_map_dfr(function(df){
      set.seed(df$i)
      n_labs <- min(20, trunc(df$n_total/16))
      labs <- lab_var %>% sample_n(n_labs) %>% droplevels()
      ctrl_sim <- tibble(lab_id = rep(labs$lab_id, each = 2, length.out = df$n_total),
                         age_days = sample(age_min:age_max, df$n_total, replace = T),
                         z_age_days = scale(age_days),
                         condition = rep(c("non-social","social"),
                                         length.out = df$n_total)) %>%
        left_join(labs) %>%
        mutate(lab_pr_helper = if_else(condition == "social", estimate, .5),
               pr_helper_rn = rnorm(df$n_total, lab_pr_helper, stdev),
               pr_helper = pmax(0, pmin(1, pr_helper_rn)),
               choice = rbinom(n = df$n_total, size = 1, p = pr_helper))
      noctrl_sim <- tibble(lab_id = rep(labs$lab_id, each = 2, length.out = df$n_total),
                           age_days = sample(age_min:age_max, df$n_total, replace = T),
                           z_age_days = scale(age_days)) %>%
        left_join(labs) %>%
        mutate(pr_helper_rn = rnorm(df$n_total, estimate, stdev),
               pr_helper = pmax(0, pmin(1, pr_helper_rn)),
               choice = rbinom(n = df$n_total, size = 1, p = pr_helper))
      
      ctrl <- glmer(choice ~ 1 + condition + z_age_days +
                      (1 + condition + z_age_days | lab_id),
                    family = "binomial", data = ctrl_sim) %>%
        summary() %>% pluck(coefficients)
      noctrl <- glmer(choice ~ 1 + z_age_days +
                        (1 + z_age_days | lab_id),
                      family = "binomial", data = noctrl_sim) %>%
        summary() %>% pluck(coefficients)
      
      df$p_ctrl <- ctrl["conditionsocial", "Pr(>|z|)"]
      df$p_noctrl <- noctrl["(Intercept)", "Pr(>|z|)"]
      return(df)
    },
    .progress = T)
  write_csv(pwr.lab_var, paste0(save_path, "glmer_sims.csv"))
}else{
  pwr.lab_var <- read_csv(paste0(save_path, "glmer_sims.csv"))
}

pwr.lab_var %>%
  group_by(n_total) %>%
  summarise(ctrl_power = mean(p_ctrl < .05), 
            noctrl_power = mean(p_noctrl < .05)) %>%
  pivot_longer(names_to = "condition", values_to = "power", contains("power")) %>%
  mutate(condition = ifelse(condition == "ctrl_power",
                            "With Social Control",
                            "No Social Control")) %>%
  ggplot(aes(x = n_total, y = power, col = condition)) + 
  geom_line() + 
...
We see overall no evident effect of lab variability on power: it seems, if anything, that the power for the no-control analysis decreased, while the power for the control analysis slightly increased.
