...
if(run_models){
  ...
  # Run all simulations
  ## Min and max for dichotomic search
  n10_infants_min <- 10
  n10_infants_max <- 50
  ...
  search.dichotomy <- function(sim.fun, sim.type, n_min, n_max, pwr_goal = .8){
    ...
    repeat{
      ...
      bfs <- future_map_dbl(1:n_sims, possibly(sim.fun, NA), n_infants = n*10,
                            .progress = T)
      bfs %>% enframe(name = NULL, value = "b.value") %>%
        write_csv(paste0(save_path, "bf_", sim.type, "_", n, ".csv"))
  ...
  n_infants_crit.ctrl <-search.dichotomy(sim.ctrl, "ctrl",
                                         n10_infants_min, n10_infants_max)
  n_infants_crit.noctrl <- search.dichotomy(sim.noctrl, "no-ctrl",
                                            n10_infants_min, n10_infants_max)
  beep(8)
}
```
...
pwr.design <- tibble(design = c("_ctrl_", "_no-ctrl_")) %>%
  mutate(n10_infants = map(design, function(design){
    list.files(save_path, pattern = paste0(design)) %>%
      str_split("[_\\.]") %>%
      map_chr(`[`, 3) %>%
      as.numeric()
  })) %>%
