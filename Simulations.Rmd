...
```{r}
n_sim <- 1000
sims <- expand_grid(n_total = seq(50,500,10), 
                    i = 1:n_sim) %>%
  mutate(idx = 1:n()) %>%
  split(.$idx) %>%
  future_map_dfr(function (df) {
    cntl_sim <- tibble(choice = c(rbinom(n = df$n_total/2, size = 1, p = pr_helper), 
                                  rbinom(n = df$n_total/2, size = 1, p = .5)), 
                       condition = c(rep("social", df$n_total/2), 
                                     rep("nonsocial", df$n_total/2)))
    nocntl_sim <- tibble(choice = rbinom(n = df$n_total, size = 1, p = pr_helper))
...
if(run_models){
  sims <- expand_grid(n_total = seq(50,500,10),
    ...
    future_map_dfr(function(df){
      ...
      ctrl_sim <- tibble(lab_id = rep(labs$lab_id, each = 2, length.out = df$n_total),
        ...
        mutate(lab_pr_helper = if_else(condition == "social", estimate, .5),
               pr_helper = rnorm(df$n_total, lab_pr_helper, stdev),
               pr_helper = pmax(0, pmin(1, pr_helper)),
               choice = rbinom(n = df$n_total, size = 1, p = pr_helper))
      noctrl_sim <- tibble(lab_id = rep(labs$lab_id, each = 2, length.out = df$n_total),
                           age_days = sample(age_min:age_max, df$n_total, replace = T),
                           z_age_days = scale(age_days)) %>%
        left_join(labs) %>%
        mutate(pr_helper = rnorm(df$n_total, estimate, stdev),
               pr_helper = pmax(0, pmin(1, pr_helper)),
               choice = rbinom(n = df$n_total, size = 1, p = pr_helper))
    ...
    .progress = T)
  write_csv(sims, paste0(save_path, "glmer_sims.csv"))
}else{
  sims <- read_csv(paste0(save_path, "glmer_sims.csv"))
}
