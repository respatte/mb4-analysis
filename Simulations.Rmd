...
```{r initialise_models full_raw_age, message=FALSE, warning=FALSE}
...
```
```{r initialise_models nointercept_raw_age, echo=FALSE, message=FALSE, warning=FALSE}
...
bf.null_age <- bf.null_age %>%
  mutate(test = "no age",
         log_bf = log1p(bf))
# Raincloud plot
ggplot(bf.null_age, aes(x = test, y = log_bf,
...
evidence <- bf.null_age %>%
  summarise(n_H1 = sum(bf >= 3),
            n_H0 = sum(bf <= 1/3))
n_H1.null_age <- evidence %>% select(n_H1) %>% as.numeric()
n_H0.null_age <- evidence %>% select(n_H0) %>% as.numeric()
```
...
pwr.design <- tibble(design = as_factor(c("_ctrl_", "_no-ctrl_"))) %>%
  ...
  mutate(beta = map2_dbl(design, n10_infants, function(design, n){
    read_csv(paste0(save_path, "bf", design, n, ".csv")) %>%
      drop_na() %>%
      deframe() %>%
      {if_else(length(.)<20, NA_real_, sum(.>3)/length(.))} # Remove values with too few samples
    }))
...
if(run_models){
  ...
  pwr.ctrl <- expand_grid(n_total = seq(50,500,10), 
    ...
    future_map_dfr(function (df) {
      cntl_sim <- tibble(choice = c(rbinom(n = df$n_total/2, size = 1, p = pr_helper_mean), 
                                    rbinom(n = df$n_total/2, size = 1, p = .5)), 
                         condition = c(rep("social", df$n_total/2), 
                                       rep("nonsocial", df$n_total/2)))
      nocntl_sim <- tibble(choice = rbinom(n = df$n_total, size = 1, p = pr_helper_mean))
...
lab_var <- tribble(
  ...
  mutate(CI_spread = (upper-lower)/2,
         stdev = CI_spread * sqrt(16) / 1.96) %>% # We assume 16 infants for each lab (conservative)
  ...
  pwr.lab_var <- expand_grid(n_total = seq(50,500,10),
    ...
    future_map_dfr(function(df){
      ...
      ctrl_sim <- tibble(lab_id = rep(labs$lab_id, each = 2, length.out = df$n_total),
                         ...
                         z_age_days = scale(age_days),
                         condition = rep(c("non-social","social"), length.out = df$n_total)) %>%
        left_join(labs) %>%
...
We see overall no evident effect of lab variability on power: it seems that the power for the no-control analysis decreased, while the power for the control analysis slightly increased.
